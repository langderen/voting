              ...
              <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <label class="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:border-primary transition-colors">
                    ...
                    >
                    <div class="ml-3">
                      <span class="block text-sm font-medium">多选投票（敬请期待）</span>
...
const minBeginDate = computed(() => getCurrentDateTime());
const minEndDate = computed(() => {
  if (vote.beginDate) {
    const beginDate = new Date(vote.beginDate);
...
const handleImageUpload = async (event: Event, index: number) => {
  ...
  if (file) {
    ...
    try {
      ...
      if (res.data && (res.data === '上传成功' || res.data.includes('成功'))) {
        const fileName = file.name;
        vote.options[index].imageUrl = `${import.meta.env.VITE_API_URL || 'https://frp-six.com:11086/api'}/image?imageName=${fileName}`;
        updatePreview();
...
const updateEndDateValidation = () => {
  updatePreview();
  if (vote.endDate && vote.beginDate && new Date(vote.endDate) < new Date(vote.beginDate)) {
    vote.endDate = vote.beginDate;
  }
};
...
const validateVote = (): boolean => {
  ...
  for (let i = 0; i < vote.options.length; i++) {
    if (!vote.options[i].text.trim()) {
      $message.warning(`请完善选项 ${i + 1} 的内容`);
      return false;
    }
  }

  if (vote.accessPermission === 'password' && !vote.password.trim()) {
    $message.warning('请设置投票密码');
    return false;
  }
...
const copyVoteLink = async () => {
  const dummyLink = `${import.meta.env.VITE_API_URL || 'https://frp-six.com:11086/api'}/vote?id=`;
  try {
