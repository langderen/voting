...
async function getpolls() {
  const res=await api.get('/poll/getvote?creatorId=' + userStore().userId)
...
const handleAvatarUpload = () => {
  $message.warning('头像上传功能待实现');
};

const handleEditVote = async (voteId: number) => {
  try {
    await api.delete('/poll?id=' + voteId);
    $message.success('投票删除成功');
    getpolls();
  } catch (error) {
    $message.error('投票删除失败，请重试');
  }
};
