...
const checkVoted = (): boolean => {
  ...
  return votedPolls[queryId.value] === true;
};

const setVoted = (): void => {
  const votedPolls = JSON.parse(localStorage.getItem('votedPolls') || '{}');
  votedPolls[queryId.value] = true;
...
async function getOptions() {
  const res=await api.get('/options?pollId=' + queryId.value)
...
async function getpoll() {
  const res=await api.get('/poll/poll?pollId=' + queryId.value)
...
const selectCandidate = (candidate: any) => {
  if (hasVoted.value) {
    $message.warning('您已经投过票了，无法重复投票');
    return;
  }
  const id = candidate.id || candidate.optionId;
  selectedCandidate.value = id;
  $message.success(`已选择：${getPrefix(candidate.optionText) || candidate.optionText || '候选人'}`);
};
...
async function addVote(optionId: number) {
  const res=await api.post('/voting', {
    pollId: queryId.value,
    optionId: optionId,
    voteAt: new Date().toISOString(),
    ipAddress: localIP.value,
  })
  return res.data;
}
...
async function loadComments() {
  try {
    const res = await api.get('/comments?id=' + queryId.value);
